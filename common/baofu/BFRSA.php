...
class BFRSA {
    ...
    function encryptedByPrivateKey($data_content) {
        ...
        while ($encryptPos < $totalLen) {
            openssl_private_encrypt(substr($data_content, $encryptPos, $this->BAOFOO_ENCRYPT_LEN), $encryptData, $this->private_key);
            $encrypted .= bin2hex($encryptData);
            $encryptPos += $this->BAOFOO_ENCRYPT_LEN;
        }
        return $encrypted;
    }
    ...
    function decryptByPublicKey($encrypted) {
        ...
        while ($decryptPos < $totalLen) {
            openssl_public_decrypt(hex2bin(substr($encrypted, $decryptPos, $this->BAOFOO_ENCRYPT_LEN * 8)), $decryptData, $this->public_key);
            $decrypt .= $decryptData;
            $decryptPos += $this->BAOFOO_ENCRYPT_LEN * 8;
        }
