 ...
 */
class Baofu2Controller extends BaseController {
    ...
    public function postData($txn_sub_type, $Encrypted_string) {
        $BFRsa = new BFRSA(\Yii::$app->params['baofu2']["pfx_file_name"], \Yii::$app->params['baofu2']["cer_file_name"], \Yii::$app->params['baofu2']["private_key_password"]); //实例化加密类。
        $Encrypted = $BFRsa->encryptedByPrivateKey($Encrypted_string); //先BASE64进行编码再RSA加密
        $PostArry = array("version" => \Yii::$app->params['baofu2']['version'],
            "terminal_id" => \Yii::$app->params['baofu2']["terminal_id"],
            "txn_type" => \Yii::$app->params['baofu2']["txn_type"],
            "txn_sub_type" => $txn_sub_type,
            "member_id" => \Yii::$app->params['baofu2']["member_id"],
            "data_type" => \Yii::$app->params['baofu2']["data_type"],
            "data_content" => $Encrypted);
        $return = HttpClient::Post($PostArry, \Yii::$app->params['baofu2']['url']);  //发送请求到宝付服务器，并输出返回结果。
